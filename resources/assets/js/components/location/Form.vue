<template>
  <form @submit.prevent="submit" method="post" action="/location">
    ...
    <div :class="['form-group', { 'has-error' : errors.province }]">
      ...
      <span v-if="errors.province" class="label label-danger">{{ errors.province[0] }}</span>
    </div>
    ...
    <div :class="['form-group', { 'has-error' : errors.regency }]">
      ...
      <span v-if="errors.regency" class="label label-danger">{{ errors.regency[0] }}</span>
    </div>
    ...
    <div :class="['form-group', { 'has-error' : errors.district }]">
      ...
      <span v-if="errors.district" class="label label-danger">{{ errors.district[0] }}</span>
    </div>
...
export default {
  ...
  methods: {
    submit(e) {
      ...
      axios
        .post(e.target.action, this.state)
        .then(response => {
